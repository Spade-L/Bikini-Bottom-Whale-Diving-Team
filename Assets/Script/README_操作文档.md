...
2. **装饰性调查点**（重要！）：每个场景放 2 个纯氛围调查物——
   ...
   > 原因：固定计数只有 23 次，不足以解锁 30 次的终门（详见配置手册第七节的总账）。
...
| 日记的最后一页 | `Dlg_roof_diary_final` | `roof_reflection` | 对话自动设置 `truth_revealed` |

**终门**：天台的入口门放在 Playground 场景的出口处，配置：
- Open Condition > Required Flags：**两个**元素：`scene_cleared_playground`、`final_door_open`
- Locked Dialogue → `Dlg_door_locked`
...
| 终门永远锁着 | 计数不足 30——检查装饰性调查点数量（第 6 步第 2 点） |
